...
| # | Disposition |
|---|-------------|
| F1 (MED, kill-switch empty-string hole) | **FIXED — and the fix's canary immediately caught a worse variant.** `tests/__init__.py` now coerces a falsy (unset OR empty) `EXPERIMENT_LOG_MIRROR_DISABLE` to `"1"`, and the new `TestSuiteKillSwitch` asserts the switch is truthy at suite runtime. That assertion FAILED on first run and exposed the real state: `python -m unittest discover tests` (the Makefile's and CI's exact invocation) imports test modules TOP-LEVEL, so `tests/__init__.py` had never executed under it — the kill switch was never active under the canonical invocation, and only the no-`.env` environment kept CI safe (F1's dev-machine scenario was live). Fix: the append-reaching modules (`test_discovery.py`, `test_mirror.py`) now `import tests` at module top, forcing the guard under every invocation style; verified green under `discover`, `make tests`, `-k` filtering, and package-style runs. Commit `3229685` briefly carried the failing canary; this commit resolves it. |
