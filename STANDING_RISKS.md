...
| SR-16 | In-process rebinding (ACCEPTED risk) | Sandbox code runs in the runner's interpreter, so an adversarially edited `research.py` could rebind `prepare.calculate_*`/runner decision functions at runtime, or mis-stamp `experiment_id` to evade the purge. Accepted because it requires a drifting agent (not an accident), every `exp:` commit diff is human-reviewable after the run, and the file-level contract already blocks the accidental version. | Monitored: the runner's per-experiment **stamp audit** counts wrong-stamp rows and surfaces `stamp_audit_violations` in the log + TSV. Earmarked hardening if it ever fires: subprocess-isolated metric read (a `python -c` that imports only `prepare`). |
