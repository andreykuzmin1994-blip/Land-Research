...
| SR-13 | Append-only experiment history | `experiment_log.tsv` and `parcel_scores` are append-only from the pipeline's perspective; the ONLY sanctioned deletion is `runner._purge_experiment_scores` reverting a non-kept experiment's own rows by `experiment_id`. The `experiment_log_mirror` table (durability mirror of the TSV, 2026-07-08) is append-only with NO sanctioned deletion at all — the purge never touches it (discard/crash/timeout rows ARE the learning record) and it is never a decision input; the TSV stays canonical. | TSV writer tests; purge tests (`TestExperimentPurge`); mirror static guards (`TestMirrorStaticGuards`) |
...
| SR-16 | In-process rebinding (ACCEPTED risk) | Sandbox code runs in the runner's interpreter, so an adversarially edited `research.py` could rebind `prepare.calculate_*`/runner decision functions at runtime, mis-stamp `experiment_id` to evade the purge, or forge/delete `experiment_log_mirror` rows — which matters most AFTER a container reclaim, when the mirror is all that survives of the history (2026-07-08). Accepted because it requires a drifting agent (not an accident), every `exp:` commit diff is human-reviewable after the run, and the file-level contract already blocks the accidental version. | Monitored: the runner's per-experiment **stamp audit** counts wrong-stamp rows and surfaces `stamp_audit_violations` in the log + TSV; TSV↔mirror divergence surfaces in `make mirror-backfill`'s summary (`mirror_only > 0` with an intact TSV = out-of-band mirror writes — run it routinely while the TSV is alive). Earmarked hardening if it ever fires: subprocess-isolated metric read (a `python -c` that imports only `prepare`). |
